...
function Public.GetMotifChapterNames( )
	...
	if (not Internal.chapters) then
		...
		local isPrePluralized = false

		if (GetCVar("Language.2") == "en") then
			isPrePluralized = GetString("SI_ITEMSTYLECHAPTER", ITEM_STYLE_CHAPTER_AXES) == "Axes"
		end
		...
		for i = 1, ITEM_STYLE_CHAPTER_MAX_VALUE do
			table.insert(Internal.chapters, {
				id = i,
				name = zo_strformat(
					-- All names, with the exception of Legs, are singular and need to be pluralized
					(not isPrePluralized and i ~= ITEM_STYLE_CHAPTER_LEGS) and "<<m:1>>" or "<<1>>",
					GetString("SI_ITEMSTYLECHAPTER", i),
					2
				),
			})
		end
